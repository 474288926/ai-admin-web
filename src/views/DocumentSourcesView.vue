      ...
      <el-form label-position="top"
        ><el-form-item
          ><template #label
            >文档源名称
            <el-tooltip
              content="给这组同步配置起一个容易识别的名称，例如公司制度共享盘；不会改变文件内容。"
        ...
        ><el-form-item
          ><template #label
            >服务端目录路径
            <el-tooltip
              content="填写后端服务器或 Docker 挂载环境中的目录，不是浏览器所在电脑的路径；目录必须位于后端允许根目录内。"
              ><el-icon><QuestionFilled /></el-icon></el-tooltip></template
          ><el-input
          ...
          ></el-form-item
        ><el-form-item
          ><el-checkbox v-model="form.recursive">同步所有子目录</el-checkbox
          ><small class="document-source-hint"
            >开启后会递归读取目录下的子文件夹；文件数量较多时首次同步会更久。</small
          ></el-form-item
        ></el-form
      >
      <template #footer
